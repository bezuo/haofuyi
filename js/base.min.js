...
;(function($){
	...
	$.fn.showTop = function (options) {
		...
		var topStart, topEnd, that = this,
			options = options ? options : null,
			directionTop = options.length != 0 ? options.parent().offset().top : 0,
			u = navigator.userAgent;

		var showHideFun = function() {
			...
			} else if(topEnd < directionTop) {
			
				options.css("display", "block");
			
			} else {
				if(options.hasClass("tab-fixed")){
		...
		var scrollBody = function (){
			topStart = $("body").scrollTop();
			if(!!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/)){ 

				window.addEventListener("touchmove", function() { 
					topEnd = $("body").scrollTop(); 
					showHideFun();
				},false);
		...
		window.addEventListener("touchstart", scrollBody, false);
